...
export const UNPUBLISHED: Record<string, string> = {
  ...
  "docs/HEALTH_MONITOR.md":
    "The runbook for the pre-liquidation warning, and the worst of the operator documents to publish. It names the threshold, the cooldown window and the deterioration margin together, which is enough to work out exactly how much warning a borrower gets and therefore how long a position can be walked toward liquidation before its owner hears anything. It also states that a failed price read is counted as a failure rather than a healthy position — useful to an operator, and a map of the seam to a reader looking for one. The publishable half is docs/product/borrow.md, which explains the health factor and what liquidation costs without the alerting timings.",
  "docs/LUCA_OPERATIONS.md":
    "The operator runbook for the AI agent: which environment variables arm it (including the model-provider keys and the service-role key whose absence makes the quota fail open), the header that authenticates the health endpoint, the three failure signatures, the SQL to read the private turn log, and the two quota levers with the number that bounds provider spend. Every line is for whoever runs the deployment; none is useful to someone using the app, and several would map the seams — the fail-open condition, the caps — to a reader looking for one.",
