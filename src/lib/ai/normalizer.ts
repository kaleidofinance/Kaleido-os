...
export const GLOSSARY: readonly string[] = [
  ...
  "TRADER SHORTHAND: 'quote me / get a quote / how much out / expected out / min received' — a live swap quote, so read the chain before answering. 'price impact / impact' and 'route / best route / aggregator' — live route data, not a fixed FAQ. 'in for / out for / rotate X into Y' — swap X for Y when both sides and an amount are explicit. 'approval / allowance / authorize / permit' — token approval; explain the approval step, never claim it was signed without a receipt.",
  "MEMECOIN / DEGEN: 'ape into X' or 'degen into X' can describe a buy only when the user also gives an amount and a spend/receive token; otherwise ask, never invent the spend. 'ape', 'ape in', 'send it', 'yolo', 'full port', 'go all in', 'WAGMI', 'NGMI', 'wen', 'diamond hands', 'paper hands', and 'bagholder' are sentiment or conviction, not executable commands. 'dump X', 'jeet X', 'sell the top', 'take profits', or 'cash out of X' can describe a sell only when amount, token, and destination are explicit; never assume USDC. 'bag / bags / holding a bag' — their balance of a token. 'moon / pump / rip / send' — sentiment, no action. 'rug / rugged / honeypot' — a scam; warn, take no action. 'CA' — a contract address. 'mcap / FDV / market cap / volume / ATH' — live data, ESCALATE. '10x / 100x' — a hoped-for multiple, not leverage. 'sniper / snipe' — a fast-buy request, but still needs explicit trade details.",
  ...
  "LIQUIDITY PROVIDER: 'LP / provide liquidity / add liquidity / seed a pool / make a market' — provideLiquidity (both token names required; one amount is enough for an existing priced pool, and the other side is calculated at live price; a new pool needs both amounts). 'range / price range / ticks / band / concentrated' — the position's price range; 'full range' — the widest. 'fee tier / 0.05% / 0.3% / 1% pool' — the pool's fee. 'collect / claim fees / harvest fees' — collectFees on a position. 'remove / pull / withdraw liquidity / close position' — removePosition. 'top up / increase / add to my position' — increasePosition. 'out of range' — the position has stopped earning. 'my LP / my positions' — the positions read.",
  "LP SHORTHAND: 'LP token / LP NFT / liquidity NFT / position NFT' — a concentrated-liquidity position, not a fungible balance. 'one-sided liquidity' — a position with one deposited leg; ask for the pair and range rather than guessing. 'range order / set a range / set my ticks' — add or increase a position with an explicit price range. 'farm fees / earn fees / fee income' — collectFees or a live position read, depending on whether the user is asking to act or inspect.",
  ...
  "BRIDGING: 'bridge / move / transfer / send / port / migrate X to <chain>' — bridge. 'bridge in / bridge out / bring X over / send cross-chain / hop to <chain>' — bridge, with the source and destination checked against the connected chain. 'cross-chain / L2 / rollup' — bridge language, not a swap. 'CCTP / native USDC bridge / Circle' — the USDC bridge. 'wrap / unwrap / convert native to wrapped / WUSDC' — wrapNative / unwrapNative. 'gas on <chain> / how do I get gas' — the native token on that chain.",
  "POINTS / REWARDS: 'points / XP / rewards / tiers / season / multiplier / streak / leaderboard / rank / eligibility / airdrop' — call getPoints; report settled points separately from rewards.availableNow and rewards.pendingPoints, and explain the five-hour hold when present. 'claim my points / claim airdrop' — there is nothing to claim; points accrue automatically. 'referral / invite / code' — waitlist referral points.",
];
