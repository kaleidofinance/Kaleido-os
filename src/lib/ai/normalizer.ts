...
export const PRODUCT_STATE: readonly string[] = [
  ...
  "Arc mainnet is live with real value. On Arc, USDC is the gas token. Swaps on Arc route through an aggregator across Arc's DEXes. Bridging USDC into or out of Arc uses Circle CCTP — a 1:1 burn-and-mint, no pool, no slippage — between Arc, Base and Ethereum; other assets bridge via an aggregator. The lending book, kfUSD and KLD staking are NOT on Arc mainnet yet: today they run on the testnets (Sepolia, Base Sepolia, BNB Smart Chain Testnet, Robinhood Chain Testnet, Arc Testnet), where tokens come from a faucet and nothing is real money.",
  ...
  "Points are live and accrue from activity — swapping on Arc mainnet, and lending, borrowing, staking and providing liquidity on the testnets — weighted by how long a position is held. They come before the token. There is no airdrop to claim.",
