...
import {
  ...
  updateBaseline,
} from "./services/userBaselineService";
...
function ChatbotWidget() {
  ...
  useEffect(() => {
    localStorage.setItem(
      "luca_agent_transactions",
      JSON.stringify(agentTransactions),
    );
  }, [agentTransactions]);
  ...
  const {
    val: usdcAllowance,
    usdrVal: usdrAllowance,
    kfusdVal: kfusdAllowance,
    usdtVal: usdtAllowance,
  } = useCheckAllowance();
  ...
  const handleAgentBridge = async (params: any) => {
    ...
    if (!quote || !quote.transactionRequest) {
      throw new Error(
        "Could not find a secure bridge route. Please try again later.",
      );
    }
  ...
  const isInfiniteApproved = () => {
    ...
    return (
      usdcAllowance > threshold ||
      usdrAllowance > threshold ||
      kfusdAllowance > threshold ||
      usdtAllowance > threshold
    );
  };
  ...
  const executeMacroStep = async (macroData: any, stepIndex: number) => {
    try {
      ...
      if (
        ...
        (macroData.action === "rebalance" ||
          macroData.action === "yield_optimization")
      ) {
        throw new Error(
          "Action aborted: Your Health Factor is too low ( < 1.1). Moving collateral now could risk liquidation.",
        );
      }

      // PHASE 4: COGNITIVE SAFETY AUDIT (Behavioral + Intent)
      const baselineRisk = validateAgainstBaseline(
        0,
        stepPattern.toToken || "UNKNOWN",
      ); // We'd pass real USD value here
      if (baselineRisk > 80) {
        throw new Error(
          `Cognitive Lock: This transaction deviates significantly from your historical behavior. Risk Score: ${baselineRisk}/100.`,
        );
      }

      const intentAudit = auditIntentDivergence(
        messages[messages.length - 1].content,
        macroData.action,
        stepPattern.toToken || "ETH",
      );
      ...
      const intendedAction =
        macroData.action === "yield_optimization"
          ? "bridge"
          : macroData.action === "arbitrage_execution"
            ? "swap"
            : "swap";

      const simResult = await simulateAgentTransaction(
        "0x...",
        "0x...",
        "0",
        11124,
      ...
      if (!simResult.success) {
        throw new Error(
          `Critical Protection: ${simResult.warning}. Simulation blocked to prevent asset drain.`,
        );
      }

      if (simResult.metrics?.hasSymmetryViolation) {
        throw new Error(
          "Sovereign Block: Symmetry Violation detected. Target attempted unauthorized state changes during calculation.",
        );
      }
      ...
      if (stepPattern.type === "swap") {
        tx = await handleAgentSwap(
          stepPattern.fromToken,
          stepPattern.toToken,
          stepPattern.amount,
        );
      } else if (stepPattern.type === "bridge") {
        toast.info(`Macro Step ${stepIndex + 1}: Bridging assets...`);
        tx = await handleAgentBridge({
          fromChain: stepPattern.fromChain,
          toChain: stepPattern.toChain,
          amount: stepPattern.amount,
          token: stepPattern.token,
        });
      } else if (
        stepPattern.type === "deposit" ||
        stepPattern.type === "lend"
      ) {
        const tokenAddr =
          stepPattern.token === "USDC"
            ? USDC_ADDRESS
            : stepPattern.token === "KALE" || stepPattern.token === "KLD"
              ? KLD_ADDRESS
              : stepPattern.token;
      ...
      if (tx && (tx.hash || tx.wait)) {
        ...
        if (!isLastStep) {
          if (permissions.speedMode) {
            ...
            toast.success(
              `Step ${stepIndex + 1} complete. Authorize next step.`,
            );
  ...
  const handleConfirmAction = async (data: any) => {
    try {
      ...
      if (data.type === "swap") {
        ...
        toast.info(
          `Initiating swap: ${amount} ${data.fromToken} → ${data.toToken}`,
        );
        ...
        if (isNative) {
          ...
          const tokenAddr =
            data.token === "USDC"
              ? USDC_ADDRESS
              : data.token === "KALE" || data.token === "KLD"
                ? KLD_ADDRESS
                : data.token;
        ...
        toast.info(
          `Initiating add liquidity: ${data.amountA} ${data.tokenA} + ${data.amountB} ${data.tokenB}`,
        );
        const tokenAAddr =
          data.tokenA === "ETH"
            ? ADDRESS_1
        ...
        toast.success(
          `Successfully added liquidity for ${data.tokenA}/${data.tokenB}!`,
        );
      } else if (data.type === "removeLiquidity") {
        toast.info(
          `Initiating remove liquidity for ${data.tokenA}/${data.tokenB}`,
        );
        const tokenAAddr =
          data.tokenA === "ETH"
            ? ADDRESS_1
        ...
        const tx: any = await createLoanListing(
          data.amount,
          Number(data.amount),
          Number(data.amount),
        ...
        toast.info(
          `Repaying loan #${data.requestId} with ${data.amount} ${data.token}...`,
        );
        const tokenAddr =
          data.token === "USDC"
            ? USDC_ADDRESS
        ...
        if (data.action === "list") {
          toast.info(
            `Creating marketplace listing for ${data.amount} ${data.token || "KALE"}...`,
          );
          const tx: any = await createLoanListing(
            data.amount,
            Number(data.amount) * 0.8,
            Number(data.amount) * 1.2,
          ...
          const tx: any = await acceptMarketplaceAd(
        ...
        const eventName =
          data.tourType === "deposit" || data.tourType === "collateral"
            ? "startCollateralTour"
            : "startDashboardTour";
        ...
        try {
          ...
          if (conversationId) {
            const existingData = localStorage.getItem(
              `kaleido_conversation_${conversationId}`,
            );
            if (existingData) {
  ...
  return (
    <div className="fixed bottom-4 right-3 z-[9999] max-w-full sm:bottom-5 sm:right-5">
      {/* Chat toggle button */}
      <div className="relative group">
        <button
          className={`flex h-12 w-12 items-center justify-center rounded-full shadow-lg transition-all duration-300 sm:h-14 sm:w-14
      ...
      {/* Chat widget */}
      {isOpen && (
        <div className="absolute bottom-14 right-1/2 flex h-[78vh] max-h-[calc(100vh-96px)] w-[calc(100vw-24px)] max-w-[550px] translate-x-1/2 flex-col overflow-hidden rounded-3xl border border-[#00ff99]/30 bg-[#1a1a2e]/95 shadow-[0_0_50px_rgba(0,0,0,0.5)] backdrop-blur-xl sm:bottom-16 sm:right-0 sm:h-[85vh] sm:max-h-[750px] sm:w-[95vw] sm:translate-x-0 md:h-[750px] chatbot-widget">
          {/* Header */}
          <div className="flex items-center justify-between gap-2 border-b border-[#00ff99]/20 bg-[#0f0f1a]/80 p-3 text-white backdrop-blur-sm sm:p-4">
            <div className="flex min-w-0 items-center gap-2 sm:gap-3">
              ...
              <div className="flex min-w-0 flex-wrap items-center gap-1.5 sm:gap-2">
                ...
                {healthFactor > 0 && (
                  ...
                  >
                    <div
                      className={`w-1 h-1 rounded-full ${healthFactor < 1.1 ? "bg-red-500" : "bg-[#00ff99]"}`}
                    ></div>
                    HF:{" "}
                ...
                <div
                  className={`text-[8px] px-1.5 py-0.5 rounded border font-bold flex items-center gap-1 ${isSynced ? "bg-[#00ff99]/15 text-[#00ff99] border-[#00ff99]/35" : "bg-gray-500/10 text-gray-500 border-gray-500/20"}`}
                  title={
                    isSynced ? "Neural Link Active" : "Neural Link Offline"
                  }
                >
                  <div
                    className={`w-1.5 h-1.5 rounded-full ${isSynced ? "bg-[#00ff99] shadow-[0_0_8px_#00ff99] animate-pulse" : "bg-gray-500"}`}
                  ></div>
                  SYNC
                </div>
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <button
                onClick={() => {
                  const isExecuting = messages.some(
                    (m) => m.functionData?.result?.status === "executing",
                  );
                  if (isExecuting) {
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between p-3 bg-[#12121f] rounded-lg border border-[#00ff99]/10">
                  <div>
                    <div className="text-xs font-bold text-white flex items-center gap-2">
                      ...
                      {isInfiniteApproved() && (
                        <span className="text-[7px] px-1 bg-[#00ff99]/20 text-[#00ff99] border border-[#00ff99]/30 rounded uppercase font-bold animate-pulse">
                          Live Active
                        </span>
                      )}
                    </div>
                    <div className="text-[10px] text-gray-400">
                      Allow Luca to spend tokens for swaps
                    </div>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      ...
                      onChange={() =>
                        setPermissions((p: AgentPermissions) => ({
                          ...p,
                          swapInfinite: !p.swapInfinite,
                        }))
                      }
                    />
                ...
                <div className="flex items-center justify-between p-3 bg-[#12121f] rounded-lg border border-[#00ff99]/10">
                  <div>
                    <div className="text-xs font-bold text-white flex items-center gap-2">
                      Yield Sentinel AI
                      <span className="text-[7px] px-1 bg-[#00ff99]/10 text-[#00ff99]/60 border border-[#00ff99]/10 rounded uppercase font-bold">
                        Background Scout
                      </span>
                    </div>
                    <div className="text-[10px] text-gray-400">
                      Luca scouts Abstract for yield opportunities
                    </div>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                ...
                <div className="flex items-center justify-between p-3 bg-[#12121f] rounded-lg border border-[#00ff99]/10">
                  <div>
                    <div className="text-xs font-bold text-white flex items-center gap-2">
                      Arbitrage Scout AI
                      <span className="text-[7px] px-1 bg-[#00ff99]/15 text-[#00ff99] border border-[#00ff99]/30 rounded uppercase font-bold">
                        PULSE Hunt
                      </span>
                    </div>
                    <div className="text-[10px] text-gray-400">
                      Luca hunts cross-chain price discrepancies
                    </div>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                ...
                <div className="flex items-center justify-between p-3 bg-[#12121f] rounded-lg border border-[#00ff99]/10">
                  <div>
                    <div className="text-xs font-bold text-[#00ff99] flex items-center gap-1">
                      <FiZap className="fill-[#00ff99]" /> Speed Mode
                    </div>
                    <div className="text-[10px] text-gray-400">
                      Skip confirmation cards; execute instantly
                    </div>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                ...
                <div className="mt-4 p-2 bg-[#00ff99]/5 rounded border border-[#00ff99]/10">
                  <p className="text-[9px] text-gray-400 text-center uppercase tracking-widest font-bold">
                    Security Note: These toggles enable "Max Allowance" for the
                    protocol contracts on first use.
                  </p>
                </div>
              </div>
            </div>
                  ...
                  {conversations.length > 0 ? (
                    <ul className="divide-y divide-white/5 chatbot-history-list">
                      {conversations.slice(0, 10).map((convo) => {
                        const previewMessage = convo.messages.find(
                          (m) => m.role === "assistant",
                        ) ||
                        ...
                        const truncatedContent =
                          previewMessage.content.length > 60
                          ...
                          >
                            <div className="flex items-start p-3">
                              ...
                              <div className="flex-1">
                                <div className="text-[10px] font-bold text-[#00ff99]/70 uppercase">
                                  {formattedDate}
                                </div>
                ...
                <div className="p-2 space-y-2 pb-4">
                  {agentTransactions.length > 0 ? (
                    agentTransactions.map((tx) => (
                      <div
                        key={tx.id}
                        className="p-3 bg-[#12121f] rounded-xl border border-white/5 hover:border-[#00ff99]/30 transition-all group"
                      >
                          ...
                          <div className="flex items-center gap-2">
                            <div
                              className={`text-[9px] font-black px-1.5 py-0.5 rounded uppercase tracking-tighter ${
                                tx.type === "SWAP"
                                  ? "bg-[#00ff99]/15 text-[#00ff99]"
                                  : tx.type === "BORROW"
                                    ? "bg-[#19aa61]/20 text-[#19aa61]"
                                    : tx.type === "LIST"
                            ...
                            <span className="text-[10px] font-bold text-gray-Profile text-white">
                              {tx.description}
                            </span>
                          </div>
                          <div
          ...
          <div
            ...
            className="flex-1 overflow-y-auto bg-[#12121f] p-3 sm:p-4 chatbot-messages-container"
          >
            ...
            {/* Loading indicator */}
            {isLoading && (
              <div className="mr-auto max-w-[92%] rounded-2xl rounded-bl-md border border-[#2a2a40] bg-[#1e1e30]/80 p-3 sm:max-w-[85%]">
                ...
                <div className="flex flex-col">
                  <div className="flex space-x-1 mb-1">
                    ...
                    <div
                      className="w-1.5 h-1.5 rounded-full bg-[#00ff99] animate-bounce"
          ...
          <div className="flex gap-2 overflow-x-auto border-t border-[#00ff99]/10 bg-[#0f0f1a]/60 px-3 py-2 sm:px-4 scrollbar-hide no-scrollbar">
            <button
          ...
          {/* Input area */}
          <div className="border-t border-[#00ff99]/20 bg-[#0f0f1a]/80 p-3 sm:p-4">
            <div className="flex items-center gap-2 sm:gap-3">
              <input
                ref={inputRef}
                type="text"
                ...
                className="chatbot-input min-w-0 flex-1 rounded-xl border border-[#00ff99]/20 bg-[#1a1a2e] px-3 py-3 text-sm text-white placeholder-gray-500 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-[#00ff99]/50 sm:px-4 sm:text-base"
                disabled={isLoading}
              />
              <button
                onClick={handleSend}
