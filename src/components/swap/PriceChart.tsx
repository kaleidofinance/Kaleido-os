...
export default function PriceChart({
  ...
  const [timeframe, setTimeframe] = React.useState<"1H" | "1D" | "1W" | "1M">(
    "1H",
  );
  ...
  React.useEffect(() => {
    ...
    const fetchSpot = async () => {
      try {
        ...
        const fromToken = ACTIVE_TOKENS.find(
          (t) => t.address.toLowerCase() === fromTokenAddress.toLowerCase(),
        );
        const toToken = ACTIVE_TOKENS.find(
        ...
        if (!val || parseFloat(val) === 0) {
          ...
          if (weth) {
            val = await getAmountsOut(
              "1",
              [fromTokenAddress, weth.address, toTokenAddress],
              fromToken.decimals,
              toToken.decimals,
            );
          }
  ...
  const activeHistory = React.useMemo(() => {
    ...
    if (spotPrice) {
      ...
      if (timeframe === "1M") volatility = 0.1;
  ...
  if (loading && !spotPrice) {
    return (
      <div
        className={`bg-black/90 backdrop-blur-sm h-[260px] w-full max-w-full rounded-2xl p-3 sm:h-[500px] sm:p-6 shadow-xl text-white relative overflow-hidden border border-[#00ff99]/20 ${className}`}
      >
      ...
      >
        <div className="flex items-center justify-center h-full">
          <p className="text-gray-400">
            No price history available for this pair
          </p>
        </div>
      </div>
    );
  ...
  const scaleX = (index: number) =>
    padding + (index / (priceData.length - 1)) * (chartWidth - 2 * padding);
  ...
  return (
    <div
      className={`bg-black/90 backdrop-blur-sm h-[320px] w-full max-w-full rounded-2xl p-3 sm:h-[500px] sm:p-6 shadow-xl text-white relative overflow-hidden border border-[#00ff99]/20 ${className}`}
    >
      {/* Retro grid overlay */}
      ...
      <div className="relative z-10">
        ...
        <div className="mb-4 flex flex-col gap-3 sm:mb-6 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-center sm:gap-6">
            <h3 className="break-words font-mono text-lg font-bold text-white sm:text-xl">
          ...
          <div className="flex items-center gap-1">
            ...
            <span
              className={`text-sm font-medium font-mono ${isPositive ? "text-[#00ff99]" : "text-red-400"}`}
            >
              {isPositive ? "+" : ""}
        ...
        {/* Chart Container */}
        <div className="relative overflow-x-auto bg-black/50 rounded-lg p-2 sm:p-4 border border-[#00ff99]/20">
          <svg
            width={chartWidth}
            height={chartHeight}
          ...
          >
            ...
            {(() => {
              ...
              const yValues = Array.from(
                { length: steps },
                (_, i) => minPrice - pricePadding + i * stepSize,
              );
                ...
                return (
                  <g key={idx}>
                    ...
                    <text
                      ...
                      y={y + 4}
                      textAnchor="start"
                      className="text-xs fill-[#00ff99] font-mono"
                      opacity="0.7"
                    >
                      {value.toFixed(4)}
                    </text>
