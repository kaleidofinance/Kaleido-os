...
export function formatPoints(value: string | number, decimals = 2): string {
  ...
  if (absNum < 1_000) {
    ...
    return (
      (num / 1_000_000_000_000).toLocaleString("en-US", {
        minimumFractionDigits: decimals,
        maximumFractionDigits: decimals,
      }) + "T"
    );
...
export const formatAmounts = (
  ...
  try {
    // READ_ONLY_CHAIN_ID rather than a parameter: the sole caller
    // (market/EnhancedCardlayout) renders rows from the P2P lending mirror,
    // which has no chainId column and is populated from the read chain only.
    // Threading a chainId in would imply a choice the data doesn't offer.
    const decimals = getTokenDecimals(READ_ONLY_CHAIN_ID, tokenAddress);
    ...
    if (unfixed) {
      ...
      return formattedAmountNum.toLocaleString("en-US", {
